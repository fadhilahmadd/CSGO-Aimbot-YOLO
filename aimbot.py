...
#net.setPreferableBackend(cv.dnn.DNN_BACKEND_OPENCV) <-- jika menggunakan cpu
...
size_scale = 2
while True:
    # Mendapatkan gambar dari screen
    ...
    for output in layerOutputs:
        for detection in output:
            ...
            if confidence > 0.7 and classID == 0: #0 adalah id untuk karakter
    ...
    # Kalkulasi jarak untuk menentukan musuh terdekat dari crosshair
    if len(indices) > 0:
        print(f"Terdeteksi:{len(indices)}")
        min = 99999
        min_at = 0
        ...
        # Jarak terdekat dari crosshair
        x = int(boxes[min_at][0] + boxes[min_at][2]/2 - frame_width/2)
        y = int(boxes[min_at][1] + boxes[min_at][3]/2 - frame_height/2) - boxes[min_at][3] * 0.5 # Untuk head shot

        # Menggerakkan mouse lalu menembak secara otomatis
        scale = 1.7
        x = int(x * scale)
        y = int(y * scale)
